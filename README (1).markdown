...
2. **Install Dependencies**:
   ...
   ```
   Required packages include `pandas`, `scikit-learn`, `streamlit`, `joblib`, and others listed in `requirements.txt`.

3. **Run the Streamlit App**:
   ```bash
   streamlit run app.py
   ```
   This launches the app locally, accessible via your browser (typically at `http://localhost:8501`).

**Note**: The dataset is not included, so running the app requires either the original data or mock inputs compatible with the model's features.